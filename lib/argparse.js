...
function except(obj, err) {
  if (this[Throw]) return this[Throw](err);
  throw err;
}
...
class Args {
  constructor(env) {
    ...
    for (const name of Object.keys(theEnv)) {
      name.replace(/^(PLUGIN|DRONE)_(.+)/, (_, scope, argName) => {
        const keyName = camelize(argName);
    ...
    for (const name of Object.keys(this.drone)) {
      ...
      this.drone[name] = coerce(this.drone[name], type);
    }
  }
  get droneEnv() {
    return this[TheEnv].drone;
  }
  get pluginEnv() {
    return this[TheEnv].plugin;
  }
  get schema() {
  ...
  dump() {
    console.log(JSON.stringify(this[TheEnv], null, 2));
    return this;
  }
  parse() {
    const res = {};
    ...
    for (const key of Object.keys(this.pluginEnv)) {
      const value = this.pluginEnv[key];
    ...
    res.drone = Object.assign({}, this.drone);
    res.ci = res.drone;
    return res;
  }
}
